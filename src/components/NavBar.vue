<template>
  <nav class="navbar navbar-expand-lg sticky-top bg-primary navbar-dark shadow-sm p-2 z-3">
    <div class="container-fluid">
      <div class="d-flex align-items-center me-auto">
        <router-link v-if="$route.name === 'PostDetail'" :to="{ name: 'PostList' }" class="me-3 py-1">
          <i class="bi bi-arrow-left fs-5 text-white"></i>
        </router-link>

        <router-link :to="{ name: 'home' }" class="navbar-brand p-0 fw-bold fs-6 text-uppercase">E-Store
          Admin</router-link>
        ...
        <ul class="navbar-nav align-items-center">
          <li class="nav-item me-2" v-if="isAuthenticated">
            <router-link :to="{ name: 'PostList' }"
              class="btn btn-sm btn-outline-light nav-link py-1">Posts</router-link>
          </li>

          <li class="nav-item me-2" v-if="!isAuthenticated">
            <router-link :to="{ name: 'register' }"
              class="btn btn-sm btn-outline-light nav-link py-1">Registrarse</router-link>
          </li>
          <li class="nav-item me-3" v-if="!isAuthenticated">
            <router-link :to="{ name: 'auth' }" class="btn btn-sm btn-light nav-link py-1 fw-bold">Iniciar
              Sesión</router-link>
          </li>
