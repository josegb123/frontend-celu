<template>
  <div class="product-search-select-container">
    ...
    <small v-if="selectedProduct" class="text-success mt-1 d-block">
      Precio: ${{ selectedProduct.precio_venta }} | Stock disponible:
...
// Asegurar que el método `getProductoById` se use correctamente
const reloadSelectedProduct = async (id: number) => {
  try {
    const product = await ProductoService.getProductoById(id)
    if (product) {
      selectedProduct.value = product
      emit('selectProduct', product) // Emitir el producto actualizado
    } else {
      console.warn(`Producto con ID ${id} no encontrado.`)
    }
  } catch (error) {
    console.error(`Error al recargar el producto con ID ${id}:`, error)
  }
}

watch(
  () => inventoryStore.saleUpdateCounter,
  (newValue, oldValue) => {
