...
watch(
  [selectedProductId, cantidad, precioCompra],
  () => {
...
<template>
  <div class="detalle-pedido-form border p-3 mb-3 rounded shadow-sm">
    <div class="row">
    ...
    <div class="row g-3 align-items-center">
      ...
      <div class="col-md-5 position-relative">
        ...
        <input
          ...
          @blur="
            () => {
              /* Keep results visible until selection or click outside */
            }
          "
        />
        <input v-if="selectedProductId" type="hidden" :value="selectedProductId" />
        <div
          v-if="showSearchResults && searchTerm.length >= 3"
          class="search-results-dropdown card position-absolute w-100 z-1000 mt-1"
        >
      ...
      <!-- Precio Unitario -->
      <div class="col-md-3">
        <label :for="`precio-compra-${index}`" class="form-label visually-hidden"
          >Precio Compra</label
        >
        <input
      ...
      <!-- Remove Button -->
      <div class="col-md-1 ms-2 d-flex justify-content-end">
        <button type="button" class="btn btn-danger btn-md" @click="removeDetalle">Eliminar</button>
      </div>
    </div>
  </div>
</template>
