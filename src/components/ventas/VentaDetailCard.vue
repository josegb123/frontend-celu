<template>
  <div>
    <div class="row mb-4 border-bottom pb-3">
      <div class="col-md-6">
        <h5 class="fw-bold text-primary">Venta #{{ ventaData.id }}</h5>
        <p class="mb-1">
          <strong>Cliente:</strong> {{ ventaData.cliente?.nombre || 'Cliente Genérico' }}
        </p>
        <p class="mb-1"><strong>Vendedor:</strong> {{ ventaData.user?.name || 'N/A' }}</p>
        <p class="mb-1">
      ...
      <div class="col-md-6 text-md-end">
        ...
        <h4 class="mt-2 text-success">Total: ${{ ventaData.total }}</h4>
        <p class="mb-0">Método de Pago: **{{ ventaData.metodo_pago }}**</p>
      </div>
    </div>
    ...
    <ul class="list-group list-group-flush mb-4 border">
      ...
      <li class="list-group-item d-flex justify-content-between bg-light">
        ...
        <span class="fs-5 fw-bold text-success">${{ ventaData.total }}</span>
      </li>
    </ul>
      ...
      <table class="table table-bordered table-sm">
        ...
        <tbody>
          <tr v-for="(item, index) in ventaData.detalles_completos" :key="index">
            <td>{{ item.nombre_producto || `Producto ID: ${item.detalle_id}` }}</td>
